...
write.csv(peaksControlsGR, "CleanpeaksControlsCelltypeperSample.csv",row.names=FALSE)
...
write.csv(peaksRBPJGR, "CleanpeaksRBPJCelltypeperSample.csv",row.names=FALSE)


proj_Clean <- addMotifAnnotations(ArchRProj = proj_Clean, motifSet = "cisbp", name = "Motif")
...
write.csv(dfUPControls, "dfDoControls.csv")
...
heatmapDoControls <- plotEnrichHeatmap(motifsDoControls, n = 7, transpose = TRUE)
figure_name =project_name
...
heatmapDoRBPJ <- plotEnrichHeatmap(motifsDoRBPJ, n = 7, transpose = TRUE)
figure_name =project_name 
